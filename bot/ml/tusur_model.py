...
# Факультеты ТУСУР согласно ТЗ и вашему config.py
TUSUR_FACULTIES = {
    "РТФ": {
...
# Список предметов из вашего bot/data/subjects.py
...
class TusurFacultyPredictor:
    ...
    def lemmatize_text(self, text):
        """Лемматизация русского текста для унификации падежей"""
        if not text:
            return ""
    ...
    def generate_training_data(self, num_samples=2000):
        """Генерация тренировочных данных для ТУСУР"""
        data = []
        ...
        for i in range(num_samples):
            ...
            # Любимые предметы (2-5 предметов)
            ...
            # Добавляем иногда случайные предметы для разнообразия
            ...
            # Нелюбимые предметы (1-4 предмета)
            ...
            # Экзамены = любимые предметы + случайные экзамены
            exams = liked.copy()
            if np.random.random() < 0.4:  # 40% шанс добавить дополнительный экзамен
            ...
            # Интересы - ключевые слова факультета + синонимы
            ...
            # Добавляем вариативность в интересы
            interest_variations = {
            ...
            # Неинтересно - ключевые слова других факультетов
    ...
    def encode_text_features(self, text, keywords):
        """Кодирование текстовых признаков на основе ключевых слов"""
        if not text:
        ...
        for keyword in keywords:
            # Проверяем точное совпадение и частичные совпадения
            score = 0
    ...
    def prepare_features(self, df):
        ...
        # Кодирование предметов (3 группы по 28 предметов каждая)
        ...
        # Кодирование текстовых полей (на основе ключевых слов)
    ...
    def train_model(self, num_samples=2000):
        ...
        # Кодирование целевых меток (факультеты)
    ...
    def predict_faculty(self, user_data):
        """Предсказание факультета для пользователя"""
    ...
    def save_model(self):
        ...
        if self.model and self.is_trained:
            model_file = self.model_path / "tusur_model.keras"  # Изменено с .h5 на .keras
...
def test_model():
    """Тестирование модели с примерами"""
    test_cases = [
        {
